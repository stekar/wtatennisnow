 ...
 */
public class AsyncATPNewsUpdateHiddenFlag extends AsyncTask<ATPNews, Void, Boolean> {
    @Override
    protected Boolean doInBackground(ATPNews... newsItems) {
        boolean isRowsUpdated = false;
        if(newsItems != null) {
            ATPNews newsItem = newsItems[0];
            isRowsUpdated = AppSqlHelper.getInstance(AppController.getInstance().getAppContext()).getATPNewsHelper().updateNewsItemHiddenFlag(newsItem, !newsItem.getIsIsHidden());
        }
