 ...
 */
public class AsyncATPNewsHiddenNewsPrefsCount extends AsyncTask<Preference, Void, Map<Preference, Long>> {
    @Override
    protected Map<Preference, Long> doInBackground(Preference ...preferenceItems) {
        final String TAG = "AsyncATPNewsHiddenNewsPrefsCount";

        Map<Preference, Long> map = new HashMap<>(2);
        long count = 0;
        try {
            if(preferenceItems != null) {
                count = AppSqlHelper.getInstance(AppController.getInstance().getAppContext()).getATPNewsHelper().queryAtpNewsHiddenCount();
    ...
    protected void onPostExecute(Map<Preference, Long> map) {
        ...
        if(pref != null) {
            String summaryText = String.format(AppController.getInstance().getResources()
                    .getString(R.string.prefs_app_news_hidden_clear_summary), count);
