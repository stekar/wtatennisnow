 ...
 */
public class AsyncATPScheduleEventsPrefsCount extends AsyncTask<Preference, Void, Map<Preference, Long>> {
    @Override
    protected Map<Preference, Long> doInBackground(Preference ...preferenceItems) {
        final String TAG = "AsyncATPScheduleEventsPrefsCount";

        Map<Preference, Long> map = new HashMap<>(2);
        long count = 0;
        try {
            if(preferenceItems != null) {
                count = AppSqlHelper.getInstance(AppController.getInstance().getAppContext()).getATPTournasHelper().queryAtpTournasEventsCount();
    ...
    protected void onPostExecute(Map<Preference, Long> map) {
        ...
        if(pref != null) {
            String summaryText = String.format(AppController.getInstance().getResources()
                    .getString(R.string.prefs_app_tourna_events_clear_summary), count);
