 ...
 */
public class AsyncATPNewsUpdateStarFlag extends AsyncTask<ATPNews, Void, Boolean> {
    @Override
    protected Boolean doInBackground(ATPNews... newsItems) {
        boolean isRowsUpdated = false;
        if(newsItems != null) {
            ATPNews newsItem = newsItems[0];
            isRowsUpdated = AppSqlHelper.getInstance(AppController.getInstance().getAppContext()).getATPNewsHelper().updateNewsItemStarFlag(newsItem, !newsItem.getIsStarred());
        }
